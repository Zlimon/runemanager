            ...
            <FormSection @submitted="addTeamMember">
                ...
                <template #form>
                    ...
                    <div v-if="availableRoles.length > 0" class="col-span-6 lg:col-span-4">
                        ...
                        <div class="relative z-0 mt-1 border border-base-300 rounded-lg cursor-pointer">
                            <button
                                ...
                                :class="{'border-t border-base-300 focus:border-none rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(availableRoles).length - 1}"
                                @click="addTeamMemberForm.role = role.key"
                            >
                                <div :class="{'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key}">
                                    <!-- Role Name -->
                                    <div class="flex items-center">
                                        <div class="text-sm text-base-content/70" :class="{'font-semibold': addTeamMemberForm.role == role.key}">
                                            {{ role.name }}
                                        </div>
                                    ...
                                    <!-- Role Description -->
                                    <div class="mt-2 text-xs text-base-content/70 text-start">
                                        {{ role.description }}
                                    </div>
                                </div>
                            </button>
                        ...
                        <div v-for="invitation in team.team_invitations" :key="invitation.id" class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                ...
                                <button
                                    ...
                                    class="cursor-pointer ms-6 text-sm text-error focus:outline-none"
                        ...
                        <div v-for="user in team.users" :key="user.id" class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                ...
                                <button
                                    ...
                                    class="ms-2 text-sm text-base-content/60 underline"
                                    @click="manageRole(user)"
                                >
                                    {{ displayableRole(user.membership.role) }}
                                </button>

                                <div v-else-if="availableRoles.length" class="ms-2 text-sm text-base-content/60">
                                    {{ displayableRole(user.membership.role) }}
                                </div>
                                ...
                                <button
                                    ...
                                    class="cursor-pointer ms-6 text-sm text-error"
                    ...
                    <div class="relative z-0 mt-1 border border-base-300 rounded-lg cursor-pointer">
                        ...
                        >
                            <div :class="{'opacity-50': updateRoleForm.role && updateRoleForm.role !== role.key}">
                                <!-- Role Name -->
                                <div class="flex items-center">
                                    <div class="text-sm text-base-content/70" :class="{'font-semibold': updateRoleForm.role === role.key}">
                                        {{ role.name }}
                                    </div>

                                    <svg v-if="updateRoleForm.role == role.key" class="ms-2 h-5 w-5 text-success" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
