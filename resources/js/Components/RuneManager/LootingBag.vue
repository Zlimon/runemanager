...
<template>
    <div v-if="!lootingBagLoading">
                ...
                <li v-for="(item, slot) in lootingBag.looting_bag" class="flex items-center justify-between">
                    <div class="relative h-20 w-20 rounded-lg border p-4 border-beige-700 dark:border-gray-700">
                        <button v-if="item.item !== null"
                                :data-tooltip-target="`lootingBag-${slot}-${item._id}-tooltip-bottom`"
                    ...
                    <div v-if="item.item !== null"
                         :id="`lootingBag-${slot}-${item.item._id}-tooltip-bottom`"
                         role="tooltip"
        ...
        <div v-else class="flex h-96 items-center justify-center">
            <p class="text-gray-500 dark:text-gray-400">No lootingBag found for this account</p>
        </div>
    </div>
