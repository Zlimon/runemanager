			...
			@forelse ($newsPosts as $post)
				@if ($loop->first) <div class="card-columns"> @endif
					<div class="card text-white bg-dark background-rivets-texture">
						...
						<div class="card-body">
							<h5 class="card-title"><a href="{{ route('news-show', $post->id) }}">{{ $post->title }}</a></h5>
							<p class="card-text">{{ $post->shortstory }}</p>
						</div>
						<div class="card-footer">
							<small><a href="{{ route('news-show', $post->id) }}">Read more <i class="fas fa-long-arrow-alt-right"></i></a></small>
						</div>
					</div>
