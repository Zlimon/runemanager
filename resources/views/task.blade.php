...
@section('content')
    ...
    <div class="task-progress-body mb-3">
        <div class="progress" style="width: 20%;">
            <div class="progress-bar bg-success" role="progressbar" style="width: {{$easyProgress}}%"
                 aria-valuenow="{{$easyProgress}}" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        <div class="progress" style="width: 20%;">
            <div class="progress-bar bg-info" role="progressbar" style="width: {{$mediumProgress}}%"
                 aria-valuenow="{{$mediumProgress}}" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        <div class="progress" style="width: 20%;">
            <div class="progress-bar bg-warning" role="progressbar" style="width: {{$hardProgress}}%"
                 aria-valuenow="{{$hardProgress}}" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        <div class="progress" style="width: 20%;">
            <div class="progress-bar bg-danger" role="progressbar" style="width: {{$eliteProgress}}%"
                 aria-valuenow="{{$eliteProgress}}" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
    </div>
                            ...
                            <div class="card task-box text-center border-warning">
                                ...
                                <div class="card task-box text-center border-danger">
                                    ...
                                    <div class="card task-box text-center">
                                        ...
                                        <div class="card-body">
                                            <img class="pixel"
                                                 src="https://www.osrsbox.com/osrsbox-db/items-icons/{{ $task->task->icon }}.png"
                                                 width="72" alt="Task '{{ $task->task->task }}' icon">
                                            <h5 class="card-title">{{ $task->task->task }}</h5>
                                            <p class="card-text">
                                                <em>{{ Helper::itemData($task->task->icon, 'examine') }}</em></p>
                                        ...
                                        <div class="card-footer">
                                            <a target="_blank" rel="noopener noreferrer"
                                               href="{{ Helper::itemData($task->task->icon, 'url') }}">"{{ Helper::itemData($task->task->icon, 'name') }}
                                                " Wiki</a>
                                        </div>
                                    </div>
                                    @endforeach
                                ...
                                @if (count($currentAccountTasks) >= 3)
                                    <button type="submit" class="btn btn-danger btn-lg btn-block mt-3" disabled>Complete
                                        more tasks!
                                    </button>
                                @else
                                    <div class="text-center mb-3">
                                        <form method="POST" action="{{ route('task') }}">
                                            ...
                                            <button type="submit" class="btn btn-primary btn-lg btn-block mt-3">Generate
                                                task
                                            </button>
                                        </form>
                                    </div>
                                            ...
                                            <tr>
                                                @switch($task->task->difficulty)
                                                    ...
                                                    <td>
                                                        ...
                                                        <img
                                                            src="https://www.osrsbox.com/osrsbox-db/items-icons/{{ $task->task->icon }}.png"
                                                            alt="Task '{{ $task->task->task }}' icon"></td>
