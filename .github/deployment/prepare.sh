...
scp -P "$ssh_port" "${ssh_jump_opts[@]}" "artifacts.tar.gz" "$ssh_user@[$ssh_host]:$remote_artifacts_path"

echo "Running the deployment script on the remote server."

ssh "$ssh_user@$ssh_host" -p "$ssh_port" "${ssh_jump_opts[@]}" "tar -xf $remote_artifacts_path .github/deployment/deploy.sh -O | bash -seo pipefail -- \"$remote_artifacts_path\" \"$base_directory\" \"$php_executable\""
